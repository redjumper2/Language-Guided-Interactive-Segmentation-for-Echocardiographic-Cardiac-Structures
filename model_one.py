"""
train_general.py — EchoVLM Training Script
Language-Guided Interactive Segmentation for Echocardiographic Cardiac Structures

Usage:
    python train_general.py --structure lv_endo --camus_root /path/to/database_nifti
                                                               --checkpoint_dir ./weights
                                                               --sam_checkpoint ./weights/sam_vit_b_01ec64.pth

Structures:
    lv_endo    — Left Ventricular Endocardium  (CAMUS label 1)
    lv_epi     — Left Ventricular Epicardium   (CAMUS label 2)
    la         — Left Atrium                   (CAMUS label 3)
...
# ---------------------------------------------------------------------------
# Argument parser — makes this script work on any machine
# ---------------------------------------------------------------------------
def parse_args():
    parser = argparse.ArgumentParser(
        description="EchoVLM — Language-Guided Cardiac Segmentation"
    )
    parser.add_argument("--structure",        type=str, default="lv_endo",
                        choices=["lv_endo", "lv_epi", "la"],
                        help="Cardiac structure to segment")
...
class CAMUSDataset(Dataset):
    ...
    """
    def __init__(self, data_root, split="train", structure="lv_endo"):
...
def extract_multiscale_features(image_encoder, x):
    """
    Extract F1 (block 3), F2 (block 7), F3 (block 11) from SAM ViT-B.

    F1 — coarse global context
    F2 — mid-level features
    F3 — fine local boundary detail
    F_neck — final neck output [B, 256, 64, 64]
    """
...
class CrossAttentionFusion(nn.Module):
    """
    At decoder stage i:
        Q = F_i W_Q        (visual features as queries)
        K = T_hat W_K      (language tokens as keys)
        V = T_hat W_V      (language tokens as values)
        A_i = softmax(Q K^T / sqrt(d_v)) V
        F_tilde_i = F_i + A_i    (residual connection)
    """
    def __init__(self, visual_dim, lang_dim=768):
        super().__init__()
        self.dv        = visual_dim
        self.lang_proj = nn.Linear(lang_dim, visual_dim)   # W_p
    ...
    def forward(self, F_i, T):
        ...
        return self.norm(F_i + A_i)   # residual: F_tilde_i = F_i + A_i
...
class MultiStageFusionDecoder(nn.Module):
    """
    Language token embeddings T are injected into visual feature maps
    at three decoder stages (F1, F2, F3) via cross-attention fusion,
    allowing semantic anatomical intent to modulate spatial features
    at both coarse and fine spatial scales.

    Resolution ladder:
        64x64   — F1 fusion (coarse)
        128x128 — F2 fusion (mid)
        256x256 — F3 fusion (fine boundary)
        512x512 — upsample
        1024x1024 — final mask output
    """
    def __init__(self, lang_dim=768):
        super().__init__()
        # Project SAM intermediate features (768 channels) to decoder dims
        self.proj1 = nn.Conv2d(768, 256, 1)
        self.proj2 = nn.Conv2d(768, 128, 1)
        self.proj3 = nn.Conv2d(768, 64,  1)

        # Cross-attention at each of the three decoder stages
        ...
        self.up1 = nn.Sequential(
            ...
            nn.BatchNorm2d(256), nn.ReLU(inplace=True)         # 64 → 128
        )
        self.up2 = nn.Sequential(
            nn.ConvTranspose2d(256 + 128, 128, 2, stride=2),
            nn.BatchNorm2d(128), nn.ReLU(inplace=True)         # 128 → 256
        )
        self.up3 = nn.Sequential(
            nn.ConvTranspose2d(128 + 64, 64, 2, stride=2),
            nn.BatchNorm2d(64), nn.ReLU(inplace=True)          # 256 → 512
        )
        self.up4 = nn.Sequential(
            nn.ConvTranspose2d(64, 32, 2, stride=2),
            nn.BatchNorm2d(32), nn.ReLU(inplace=True)          # 512 → 1024
        )
        self.final = nn.Conv2d(32, 1, kernel_size=1)           # binary logit
...
# ---------------------------------------------------------------------------
# Loss — Equation 4 from paper: L = L_BCE + lambda * (1 - DSC)
# ---------------------------------------------------------------------------
...
def main():
    ...
    log_path = os.path.join(args.checkpoint_dir, f"train_log_{args.structure}.txt")

    print(f"--- EchoVLM Training ---")
    ...
    train_set    = CAMUSDataset(args.camus_root, split="train", structure=args.structure)
    val_set      = CAMUSDataset(args.camus_root, split="val",   structure=args.structure)
    ...
    best_val_dsc = 0.0
    print(f"\nStarting training ({args.structure})...\n")

    for epoch in range(args.epochs):

        # Unfreeze BiomedBERT after warmup
        ...
        if val_dsc > best_val_dsc:
            ...
            save_path = os.path.join(args.checkpoint_dir, f"best_model_{args.structure}.pth")
            torch.save({
                "epoch":        epoch,
        ...
        if (epoch + 1) % args.backup_every == 0:
            backup_path = os.path.join(args.checkpoint_dir,
                                       f"checkpoint_{args.structure}_epoch{epoch+1}.pth")
            torch.save({
                "epoch":        epoch,
                "decoder":      decoder.state_dict(),
                "bert_model":   bert_model.state_dict(),
    ...
    print(f"\nTraining complete. Best Val DSC: {best_val_dsc:.4f}")

    # Verify checkpoint
    print("\nVerifying checkpoint...")
    ckpt = torch.load(os.path.join(args.checkpoint_dir, f"best_model_{args.structure}.pth"),
                      map_location="cpu")
