"""
train_la.py — EchoVLM Left Atrium Training Script
Language-Guided Interactive Segmentation for Echocardiographic Cardiac Structures

Trains a model to segment the Left Atrium (LA) from echocardiographic images
using natural language prompts.

Usage:
    python train_la.py --camus_root /path/to/database_nifti
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="EchoVLM — Left Atrium Segmentation Training"
    )
...
class CAMUSDataset(Dataset):
    ...
    """
    def __init__(self, data_root, split="train", structure="la"):
...
def main():
    ...
    log_path = os.path.join(args.checkpoint_dir, "train_log_la.txt")

    print("--- EchoVLM Left Atrium Training ---")
    ...
    for epoch in range(args.epochs):
        ...
        if val_dsc > best_val_dsc:
            ...
            save_path = os.path.join(args.checkpoint_dir, "best_model_la.pth")
            torch.save({
                "epoch":        epoch,
        ...
        if (epoch + 1) % args.backup_every == 0:
            backup_path = os.path.join(args.checkpoint_dir,
                                       f"checkpoint_la_epoch{epoch+1}.pth")
            torch.save({
                "epoch":        epoch,
                "decoder":      decoder.state_dict(),
                "bert_model":   bert_model.state_dict(),
    ...
    print(f"\nLeft Atrium training complete. Best Val DSC: {best_val_dsc:.4f}")

    print("\nVerifying best_model_la.pth...")
    ckpt = torch.load(os.path.join(args.checkpoint_dir, "best_model_la.pth"),
                      map_location="cpu")
